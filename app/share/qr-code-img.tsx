...
function drawLocators(
  ...
  function drawOneLocator(leftXModules: number, topYModules: number) {
    ...
    drawRoundRect(context, margin + leftXPixels, margin + topYPixels, dimPixels, dimPixels, radius, MODULE_COLOR)

    leftXPixels += modulePixelSize
    topYPixels += modulePixelSize
    dimPixels -= 2 * modulePixelSize
    drawRoundRect(context, margin + leftXPixels, margin + topYPixels, dimPixels, dimPixels, radius, BACKGROUND_COLOR)
  ...
  for (let row = 0; row < srcNumRows; row += 1) {
    ...
    for (let col = 0; col < DINO_WIDTH; col += 1) {
      if (srcArray[whichByte] & mask) {
        ...
        context.fillRect(Math.floor(pixelX), Math.floor(pixelY), Math.ceil(scaleX), Math.ceil(scaleY))
      }
...
function drawCenterImage(
  ...
  if (canvasSize / 2 < dinoWidthPx + dinoBorderPx || canvasSize / 2 < dinoHeightPx + dinoBorderPx) return
  ...
  const backgroundRight = Math.floor((destX + dinoWidthPx + dinoBorderPx + modulePixelSize - 1) / modulePixelSize) * modulePixelSize
  const backgroundBottom = Math.floor((destY + dinoHeightPx + dinoBorderPx + modulePixelSize - 1) / modulePixelSize) * modulePixelSize

  context.fillStyle = BACKGROUND_COLOR
  context.fillRect(backgroundLeft, backgroundTop, backgroundRight - backgroundLeft, backgroundBottom - backgroundTop)
  ...
  drawDinoPixelData(context, DINO_BODY, DINO_BODY_HEIGHT, DINO_HEAD_HEIGHT, destX, destY, scaleX, scaleY)
}
...
function renderQRCodeChromiumStyle(
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      if (originalX < 0 || originalY < 0 || originalX >= originalSize || originalY >= originalSize) continue
  ...
  return (
    <div className="text-share-qr-card">
      {status === 'error' ? (
        ...
        <canvas ref={canvasRef} aria-label="QR Code" className="text-share-qrcode" />
      )}
      {status === 'loading' ? (
