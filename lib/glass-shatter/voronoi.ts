...
function clipPolygon(
  ...
  const clipEdges: {
  ...
  }[] = [
    {
      inside: (p) => p[0] >= 0,
      intersect: (a, b) =>
        [0, a[1] + ((0 - a[0]) / (b[0] - a[0])) * (b[1] - a[1])] as [number, number],
    },
    {
      inside: (p) => p[0] <= width,
      intersect: (a, b) =>
        [width, a[1] + ((width - a[0]) / (b[0] - a[0])) * (b[1] - a[1])] as [number, number],
    },
    {
      inside: (p) => p[1] >= 0,
      intersect: (a, b) =>
        [a[0] + ((0 - a[1]) / (b[1] - a[1])) * (b[0] - a[0]), 0] as [number, number],
    },
    {
      inside: (p) => p[1] <= height,
      intersect: (a, b) =>
        [a[0] + ((height - a[1]) / (b[1] - a[1])) * (b[0] - a[0]), height] as [number, number],
    },
  ];
...
function applySecondaryFracture(
  ...
  for (const cell of cells) {
    ...
    const shouldSplit = Math.random() < splitChance && cell.vertices.length >= 4;
...
function growBranch(
  ...
  const forkChance = depth === 0 ? 0.38 : depth === 1 ? 0.28 : depth === 2 ? 0.18 : 0.08;
