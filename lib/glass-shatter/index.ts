...
function createFreezeLayer(source: HTMLCanvasElement): HTMLCanvasElement {
  ...
  freezeLayer.style.cssText =
    "position:fixed;inset:0;z-index:99997;width:100vw;height:100vh;pointer-events:none;";
...
async function captureViewport(
  ...
  croppedCanvas
    ...
    .drawImage(
      fullScreenshot,
      0,
      Math.round(window.scrollY * realScaleY),
      Math.round(width * realScaleX),
      Math.round(height * realScaleY),
      0,
      0,
      croppedCanvas.width,
...
export async function triggerShatter(options?: ShatterOptions): Promise<void> {
  ...
  try {
    ...
    const { cells } = voronoiModule.generateVoronoiCells(fragmentCount, width, height, origin);
    ...
    const crackCanvas = await engineModule.runCrackPhase(crackTree, width, height, {
      origin,
